...
- **そっと直す訂正カード**：間違えたら日本語で理由付き解説。引用した文の間違いは全部直し、正しい文には付けない。会話は止めない
- **言い直し練習**：直された文を、その場で声（打ち込みでも）で言い直して答え合わせ。見て終わりにしない
- **訳**：相手の返事は「訳」を押すと日本語で読める
- **聞き取りモード**：🎧 で相手の英文を隠して、まず耳だけで聞く。🐢 でゆっくり聞き直せる
- **日本語で聞いてOK**：「〜って英語でなんて言うの？」と聞くと、相手役のまま言い方を教えてくれる
- **フレーズ引き出し**：詰まったらタップして入力欄へ。自分の答えノートの答えも出る
- **会話のまとめ**：その会話で直された言い方を並べて、もう一度聞ける
- **音声入力／読み上げ**：Web Speech API + クラウドTTS（任意）

### 自分の答えノート
- フェスで必ず聞かれる10の質問（名前・出身・仕事・初めて？・好きな音楽・福岡のおすすめ…）
- 答えを日本語で書くと、紬が口に出して言える英語（1〜2文）にしてくれる。言いたいことは変えない
- 作ったら声に出して練習し、復習カードにも入る
...
- **間隔反復（SRS）**：会話で直された表現・フェスのフレーズ（必修から1日8枚ずつ）・自分の答えを、Leitner box で7段階スケジュール。日本語を見て、声に出して答える
- **電波がなくても使える**：一度開けば、アプリ本体と紬の声（約5MB・1回だけ）を保存しておく。会場で電波が悪くても、フレーズ帳・自分の答えノート・復習が紬の声で使える（会話は AI とのやり取りなので電波が必要）
...
components/
  ...
  tsumugi/
    ...
    MicButton.tsx          音声入力
    SayItPractice.tsx      声に出す練習（言い直し・暗記カード・自分の答え）
  screens/                 各画面
...
lib/
  ...
  srs.ts                   間隔反復スケジューラ
  review.ts                復習に出すカードを並べる（直された文・フェスのフレーズ・自分の答え）
  speechMatch.ts           言った英語とお手本の答え合わせ（聞き違いには寛容、文法の抜けは見逃さない）
  answerQuestions.ts       自分の答えノートの10の質問
  myAnswers.ts             下書きを、口に出せる英語の答えにしてもらう
